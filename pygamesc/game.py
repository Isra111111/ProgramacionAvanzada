...
bala_img = pygame.image.load('assets/sprites/purple_ball.png')  # La imagen de la bala
...
def generar_meteoritos():
    if random.random() < 0.01:  # 1% de probabilidad por frame
        meteorito = pygame.Rect(random.randint(0, w-50), 0, 50, 50)
...
def mover_meteoritos():
    ...
    for meteorito in meteoritos:
        meteorito.y += 5  # Mover el meteorito hacia abajo
        if meteorito.y > h:
            meteoritos.remove(meteorito)  # Eliminar meteorito cuando sale de la pantalla
...
def update():
    ...
    # Dibujar meteoritos con la misma imagen que la bala
    for meteorito in meteoritos:
        pantalla.blit(bala_img, (meteorito.x, meteorito.y))  # Usar la imagen de la bala para el meteorito
...
def main():
    global salto, en_suelo, bala_disparada, mover_izquierda, mover_derecha
    ...
    while correr:
        ...
        if not pausa:
            # Movimiento solo cuando las teclas de izquierda o derecha son presionadas
            if mover_izquierda:
                jugador.x -= 5
            if mover_derecha:
                jugador.x += 5
